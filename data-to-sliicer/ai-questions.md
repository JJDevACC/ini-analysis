...
### 7. Value discrepancy between our Python output and Craig's example CSV
Our interpolated and PI summary methods produce values that are close to each other (within ~1-4%) but differ from Craig's example CSV by 3-20%, with inconsistent direction (sometimes higher, sometimes lower). This rules out a simple unit conversion issue. Possible causes: (a) we're hitting the dev PI server (`masterpidvapp`) while Craig may have used production (`MASTERPIAPP`), (b) data corrections/recalculations since Craig's pull, (c) different tag or data source, (d) client-side manipulation in Craig's R workflow that we don't have visibility into. Need Craig to confirm: which PI server he used, the exact tag name, and whether any additional transformations are applied in his R workflow beyond what's in the piwebapi library.
